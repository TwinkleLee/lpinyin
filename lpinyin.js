...
var pinyin_dict_notone = {
  ...
  xie:
    "些解协写血谢械鞋胁斜携懈契卸谐泄蟹邪歇泻屑挟燮榭蝎撷偕亵楔颉缬邂鲑瀣勰榍薤绁渫廨獬躞",
...
//判断是否包含中文
var isCN = function (text) {
  var regEx = /[^\u4e00-\u9fa5\uf900-\ufa2d]/g;
  return !regEx.test(text);
};

//传入一个字符,中文则返回其拼音或首字母的数组,否则返回其自身
function get_char_pinyin_arr(char, isAcronym) {
  var rdata_arr = [];
  if (isCN(char)) {
    for (var i in pinyin_dict_notone) {
      //找到拼音
      if (pinyin_dict_notone[i].indexOf(char) != -1) {
        rdata_arr.push(isAcronym ? i.slice(0, 1) : i);
      }
    }
  } else {
    rdata_arr.push(char);
...
//返回文本拼音或首字母的二维数组 如"你A乐爱"=>[["n"],["a"],["y","l"],["a"]]
function get_d2_array(text, isAcronym) {
  text = text.toLowerCase();
  var d2_array = [];
  for (var i in text) {
    var py = get_char_pinyin_arr(text[i], isAcronym);
    py = Array.from(
      new Set(
        py.map((item) => {
          return item;
...
//判断filterText是否能模糊查询到二维数组d2_array
function check(d2_array, filterText) {
  // console.log("check",text,filterText)
  //输出d2_array所有可能拼音的数组
  var all_possible_arr = [""];
  for (var i = 0; i < d2_array.length; i++) {
    var copy_arr = Object.assign([], all_possible_arr);
    var new_arr = [];
    for (var j = 0; j < d2_array[i].length; j++) {
      new_arr = new_arr.concat(
        ...copy_arr.map((item) => item + d2_array[i][j])
      );
    }
    all_possible_arr = new_arr;
  }
  // console.log("all_possible_arr", all_possible_arr);

  for (var possible_text of all_possible_arr) {
    if (possible_text.indexOf(filterText) != -1) {
      // console.log("匹配成功");
      return true;
    }
  }
...
function lpinyin(obj_arr, attr) {
  //初始化
  this.obj_arr = obj_arr;
  //为obj_arr中每一项的attr生成首字母的二维数组
  this.all_d2_array_acronym = obj_arr.map((item) =>
    get_d2_array(item[attr], true)
  );
  // console.log("all_d2_array_acronym", this.all_d2_array_acronym);

  //为obj_arr中每一项的attr生成拼音的二维数组
  this.all_d2_array_pinyin = obj_arr.map((item) =>
    get_d2_array(item[attr], false)
  );
  // console.log("all_d2_array_pinyin", this.all_d2_array_pinyin);
}

//过滤符合条件的内容
//acronym_type 0(默认):首字母+全拼 1:仅首字母 2:全拼
lpinyin.prototype.query = function (filterText, acronym_type) {
  filterText = filterText.toLowerCase();
  switch (acronym_type) {
    case 1:
      return this.obj_arr.filter((obj, index) =>
        check(this.all_d2_array_acronym[index], filterText)
      );
    case 2:
      return this.obj_arr.filter((obj, index) =>
        check(this.all_d2_array_pinyin[index], filterText)
      );
    default:
      return this.obj_arr.filter(
        (obj, index) =>
          check(this.all_d2_array_acronym[index], filterText) ||
          check(this.all_d2_array_pinyin[index], filterText)
      );
